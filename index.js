...
Auth.prototype.authenticate = function(user, password, callback) {
  ...
  if (cacheUser) {
    ...
    return callback(null, [
      cacheUser.cn,
      ...(cacheUser._groups
        ? [].concat(cacheUser._groups).map(group => group.cn)
        : []),
      ...(cacheUser.memberOf
        ? []
            .concat(cacheUser.memberOf)
            .map(groupDn => rfc2253.parse(groupDn).get("CN"))
        : [])
    ]);
  }
  ...
  LdapClient.authenticateAsync(user, password)
    .then(ldapUser => {
      ...
      return [
        ...
        ...(ldapUser._groups
          ? [].concat(ldapUser._groups).map(group => group.cn)
          : []),
        ...(ldapUser.memberOf
          ? []
